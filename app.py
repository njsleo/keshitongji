...
def render_verification_sheet(worksheet, grid_df, class_name, f_start, f_end):
    ...
    for r in range(R):
        ...
        for c in range(C):
            ...
            if c < 2 or r < 2: cell.font = Font(bold=True) # 前两列和前两行加粗
    ...
    for r in range(R):
        ...
        for c in range(C):
            ...
            cell.alignment = center_align
            cell.border = thin_border
            if c < 2 or r < 2: cell.font = Font(bold=True) 
    ...
    actual_data_start_row = act_start_row + 3 # 跳过实际区的日期和星期两行表头，从第3行开始比较
...
if st.session_state['all_sheets'] is not None:
    ...
    if st.session_state.get('batch_export_mode'):
        ...
        if "班主任核对表" in export_mode_type:
            ...
            with st.spinner('正在切片全校各班的标准排课数据，并粉碎多余空行...'):
                for s_name in valid_classes_to_search:
                    ...
                    if target_block:
                        ...
                        if cleaned_rows:
                            ...
                            # 把第一行的日期格式化得干净漂亮 (抹掉后面的 00:00:00)
