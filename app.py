...
if st.session_state['all_sheets'] is not None:
    ...
    if st.session_state.get('batch_export_mode'):
        ...
        if "班主任核对表" in export_mode_type:
            ...
            with st.spinner('正在切片全校各班的标准排课数据，并彻底粉碎隐藏的空行与格式杂质...'):
                for s_name in valid_classes_to_search:
                    ...
                    for row_idx in range(len(s_df)):
                        # 【核心防线1】：全行无死角扫描，遇到“第X周”立刻切断当前块，丢弃剩余空白！
                    ...
                    if target_block:
                        ...
                        for i in range(len(grid_df)):
                            ...
                            # 剔除整个课表全空的废行
                            course_vals = row_vals[2:]
                            courses_empty = all((not v or v in ['0', '0.0', 'nan', 'none', 'nat', 'null']) for v in course_vals)
                            ...
                            # 如果全都没内容，滚蛋
                            if p_empty and t_empty and courses_empty:
                                continue
                            
                            # 如果左边没内容，除非它是星期行，否则也滚蛋
                            if p_empty and t_empty:
                                if any("星期" in v for v in course_vals):
                                    cleaned_rows.append(i)
                                continue
                                
                            # 剔除带有备注、签章的废行
                            if re.search(r'(第[一二三四五六七八九十0-9\s]+周|日期|交表|备注|签章|签字|核对)', "".join(row_vals)):
                                continue
            ...
            if class_grids:
                if "ZIP" in export_mode_type:
                    ...
                    st.success(f"🎉 成功生成！已为您把全校 {len(class_grids)} 个班级的独立表格打包成 ZIP！空行已被完全销毁！")
                    st.download_button(
                ...
                for s_name in valid_classes_to_search:
                    ...
                    for col_idx in range(start_idx, end_idx + 1):
                        ...
                        for row_idx in range(len(s_df)):
                            ...
                            # 【同款雷达防线】：整行扫描，遇到“第X周”立刻掐断这一周期的抓取，杜绝尾部空白
                            row_all_text = "".join([str(x) for x in s_df.iloc[row_idx, :].values])
                            if re.search(r'第[一二三四五六七八九十0-9\s]+周', row_all_text):
                                current_date = None
                                continue
