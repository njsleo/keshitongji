...
if st.session_state['all_sheets'] is not None:
    ...
    if st.session_state.get('batch_export_mode'):
        ...
        if "班主任核对表" in export_mode_type:
            ...
            with st.spinner('正在切片全校各班的标准排课数据，并彻底粉碎隐藏空行...'):
                for s_name in valid_classes_to_search:
                    ...
                    if target_block:
                        ...
                        for i in range(len(grid_df)):
                            ...
                            p_val = row_vals[0]
                            t_val = row_vals[1]
                            if bool(re.search(r'(第.*周|日期|交表|备注|签章|签字)', p_val + t_val)):
                                continue
            ...
            if class_grids:
                if "ZIP" in export_mode_type:
                    ...
                    st.info("💡 解压这个 ZIP 文件后，里面会有每个班级独立的 Excel 文件，您可以直接拖进微信群发给各个班主任！里面已经去除了所有空白幽灵行，干净无暇！")
                else:
