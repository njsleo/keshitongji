...
def render_verification_sheet(worksheet, grid_df, class_name, f_start, f_end):
    ...
    std_fill = PatternFill(start_color="1E3C72", end_color="1E3C72", fill_type="solid") # 深蓝
    act_fill = PatternFill(start_color="D35400", end_color="D35400", fill_type="solid") # 警示橙
    center_align = Alignment(horizontal='center', vertical='center', wrap_text=True)
    thin_border = Border(left=Side(style='thin'), right=Side(style='thin'), top=Side(style='thin'), bottom=Side(style='thin'))
    ...
    cell = worksheet.cell(row=std_start_row, column=1, value="⬇️ === 标准课表 (原定计划) === ⬇️")
    ...
    for r in range(R):
        worksheet.row_dimensions[std_start_row + 1 + r].height = 25
        for c in range(C):
            val = grid_df.iloc[r, c]
            if pd.isna(val): val = ""
            cell = worksheet.cell(row=std_start_row + 1 + r, column=c + 1, value=str(val))
            cell.alignment = center_align
    ...
    cell = worksheet.cell(row=act_start_row, column=1, value="✍️ === 实际上课核对 (请在此区域修改，与标准不同将自动标红) === ✍️")
    cell.font = header_font
    cell.fill = act_fill
    cell.alignment = center_align
    ...
    for r in range(R):
        worksheet.row_dimensions[act_start_row + 1 + r].height = 25
        for c in range(C):
            val = grid_df.iloc[r, c]
            if pd.isna(val): val = ""
            cell = worksheet.cell(row=act_start_row + 1 + r, column=c + 1, value=str(val))
            cell.alignment = center_align
            cell.border = thin_border
            if c < 2: cell.font = Font(bold=True) 
    ...
    actual_data_start_row = act_start_row + 1
    actual_data_end_row = act_start_row + R
    std_data_start_row = std_start_row + 1
    ...
    rule = FormulaRule(formula=[formula], stopIfTrue=True, fill=red_fill, font=red_font)
...
if st.session_state['all_sheets'] is not None:
    ...
    if st.session_state.get('batch_export_mode'):
        ...
        if "班主任核对表" in export_mode_type:
            ...
            st.markdown(f"<h3 style='color:#1e3c72;'>📝 班主任自动标红核对表 批量生成 📅 ({f_start} 至 {f_end})</h3>", unsafe_allow_html=True)
            ...
            with st.spinner('正在切片全校各班的标准排课数据，并嵌入防呆标红公式...'):
                for s_name in valid_classes_to_search:
                    ...
                    if target_block:
                        ...
                        # 【核心修复】：智能清理多余的空行和“日期/第几周”杂质行
                        cleaned_rows = []
                        for i in range(len(grid_df)):
                            period_val = str(grid_df.iloc[i, 0]).strip()
                            time_val = str(grid_df.iloc[i, 1]).strip()
                            
                            # 如果节次和时间全都为空，认为是没用的空行
                            is_empty = (not period_val or period_val in ['0', '0.0', 'nan', 'None']) and (not time_val or time_val in ['0', '0.0', 'nan', 'None'])
                            # 如果带有“第x周”或者“日期”，说明是原表留下的杂质行
                            is_meta = bool(re.search(r'(第.*周|日期|交表)', period_val + time_val))
                            
                            if not is_empty and not is_meta:
                                cleaned_rows.append(i)
                                
                        if cleaned_rows:
                            grid_df = grid_df.iloc[cleaned_rows].reset_index(drop=True)
            ...
            if class_grids:
                ...
                if "ZIP" in export_mode_type:
                    ...
                    st.success(f"🎉 成功生成！已为您把全校 {len(class_grids)} 个班级的独立表格打包成 ZIP！")
                    st.download_button(
                        label=f"⬇️ 下载 ZIP 压缩包 (发群专用)",
                        data=zip_data, 
                        file_name=f"班主任课时核对表大合集_{f_start}至{f_end}.zip",
                        mime="application/zip"
                    )
                    st.info("💡 解压这个 ZIP 文件后，里面会有每个班级独立的 Excel 文件，您可以直接拖进工作群，让各个班主任去领自己班的！")
                else:
        ...
        # 2. 教师专属课表提取逻辑
            ...
            with st.spinner('正在像雷达一样扫描全校所有班级数据...'):
                for s_name in valid_classes_to_search:
                    s_df = st.session_state['all_sheets'][s_name]
                    if len(s_df.columns) <= max(p_idx, t_idx, end_idx): continue
