...
def render_verification_sheet(worksheet, grid_df, class_name, f_start, f_end):
    ...
    for r in range(R):
        ...
        for c in range(C):
            ...
            # 【核心权限开放】：只允许班主任修改真正的课程格子 (剔除左侧表头和上方日期表头)
            if r >= 2 and c >= 2:
        ...
        if "班主任核对表" in export_mode_type:
            ...
            if class_grids:
                if "ZIP" in export_mode_type:
                    ...
                    st.success(f"🎉 成功生成！已为您把全校 {len(class_grids)} 个班级的独立核对表打包成 ZIP！【带自动锁定与红底黑字警示】")
                    st.download_button(
                        label=f"⬇️ 立即下载 ZIP 压缩包 (发群专用)",
                        data=zip_data, 
                        file_name=f"班主任自动标红核对表大合集_{f_start}至{f_end}.zip",
                        mime="application/zip"
                    )
                    st.info("💡 解压这个 ZIP 文件后，里面全是独立的 Excel 文件。标准课表区域已经被锁定，班主任只能在下方修改，修改后格子会变成极其醒目的【大红色背景+纯黑粗体字】！(解锁密码:123)")
                else:
