...
if st.session_state['all_sheets'] is not None:
    ...
    if st.session_state.get('batch_export_mode'):
        ...
        if "班主任核对表" in export_mode_type:
            ...
            with st.spinner('正在切片全校各班的标准排课数据，并触发【终极闸门】隔离多余空行...'):
                for s_name in valid_classes_to_search:
                    ...
                    for row_idx in range(len(s_df)):
                        ...
                        row_header = str(s_df.iloc[row_idx, 0]) + str(s_df.iloc[row_idx, 1]) + val_str
                        
                        # 【终极防线】：只要看见“第几周”，立刻拉下闸门，丢弃后面的所有废行！
                        m_week = re.search(r'第[一二三四五六七八九十0-9]+周', row_header)
                        m_date = re.search(r'(\d{4})[-/年\.](\d{1,2})[-/月\.](\d{1,2})', val_str)
                        
                        if m_week:
                            if current_block is not None and current_date is not None:
                                blocks.append((current_date, current_block))
                            current_block = None # 强行关闭收集通道
                            current_date = None
                            continue
                ...
                for s_name in valid_classes_to_search:
                    ...
                    for col_idx in range(start_idx, end_idx + 1):
                        ...
                        for row_idx in range(len(s_df)):
                            ...
                            # 【同款防线】：遇到第几周，立刻切断通道
                            row_header = str(s_df.iloc[row_idx, 0]) + str(s_df.iloc[row_idx, 1]) + val_str
                            if re.search(r'第[一二三四五六七八九十0-9]+周', row_header):
                                current_date = None
                                continue
                        ...
                        if len(date_range) >= 1:
                            ...
                            for col_idx in range(start_idx, end_idx + 1):
                                ...
                                for row_idx in range(len(display_df)):
                                    ...
                                    row_header = str(display_df.iloc[row_idx, 0]) + str(display_df.iloc[row_idx, 1]) + val_str
                                    if re.search(r'第[一二三四五六七八九十0-9]+周', row_header):
                                        current_date = None
                                        continue
