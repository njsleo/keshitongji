...
# 【神级核对表生成器】：生成带条件格式（自动变红）的班主任专用核对表
        ...
        for class_name, grid_df in class_grids.items():
            ...
            for r in range(R):
                ...
                for c in range(C):
                    ...
                    cell.alignment = center_align
                    cell.border = thin_border
                    if c < 2: cell.font = Font(bold=True) # 前两列固定节次加粗
            ...
            # 4. 添加超强条件格式 (自动变红)
            # 作用范围：实际核对区的数据列 (第3列到最后一列，即 C 列开始)
...
if st.session_state['all_sheets'] is not None:
    ...
    if st.session_state.get('batch_export_mode'):
        ...
        if "班主任核对表" in export_mode_type:
            ...
            st.markdown(f"<h3 style='color:#1e3c72;'>📝 班主任专属：自动标红核对表批量生成 📅 ({f_start} 至 {f_end})</h3>", unsafe_allow_html=True)
            ...
            with st.spinner('正在切片全校各班的标准排课数据，嵌入防呆标红代码...'):
            ...
            if class_grids:
                ...
                st.success(f"🎉 成功生成了全校 {len(class_grids)} 个班级的【带自动标红功能】课时核对表！")
                st.download_button(
                    label=f"⬇️ 下载发给班主任：《全校课时核对表大合集》",
                    data=excel_data, 
                    file_name=f"班主任课时核对表_{f_start}至{f_end}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                )
                st.info("💡 下载打开后，每个班级都是一个独立的底部标签页。下方的【实际上课核对区】已经内置了强大的条件格式，只要班主任填写的老师与上面的标准计划不符，单元格就会自动变成红色警示！")
            else:
                st.warning(f"😔 在您选定的日期 {f_start} 到 {f_end} 之间，没有在表格中找到对应的排课区域。")

        # 2. 教师专属课表提取逻辑 (与之前功能保持不变，支持分表或单表垂直合并)
        else:
            ...
            if all_teachers_schedule:
                ...
                info_msg = "💡 您选择的是【单表合并打印】模式，打开下载的文件，**所有老师的课表将从上到下依次排开，中间留有间隔**，直接点击打印即可全部输出！" if "单表" in export_mode_type else "💡 您选择的是【分表排版】模式，打开下载的文件，每位老师都拥有一个独立的底部标签页（Sheet）。"
                st.info(info_msg)
            else:
