...
def render_verification_sheet(worksheet, grid_df, class_name, f_start, f_end):
    ...
    actual_data_start_row = act_start_row + 3 # 跳过表头，防止误伤
...
if st.session_state['all_sheets'] is not None:
    ...
    if st.session_state.get('batch_export_mode'):
        ...
        if "班主任核对表" in export_mode_type:
            ...
            with st.spinner('正在切片全校各班的排课数据，并彻底粉碎隐藏的空行与格式杂质...'):
                for s_name in valid_classes_to_search:
                    ...
                    if target_block:
                        ...
                        for i in range(len(grid_df)):
                            if i == 0:
                                cleaned_rows.append(i) # 强行保住第一行的日期
                                continue
                                
                            p_val = str(grid_df.iloc[i, 0]).strip().lower()
                            t_val = str(grid_df.iloc[i, 1]).strip().lower()
                            
                            # 如果左侧两列（节次/时间）全都是空，说明这行大概率是废行
                            p_empty = not p_val or p_val in ['0', '0.0', 'nan', 'none', 'nat', 'null']
                            t_empty = not t_val or t_val in ['0', '0.0', 'nan', 'none', 'nat', 'null']
                            
                            if p_empty and t_empty:
                                # 检查这行是否是写着“星期一”之类的表头
                                row_vals = [str(x) for x in grid_df.iloc[i, 2:].values]
                                if any("星期" in v for v in row_vals):
                                    cleaned_rows.append(i)
                                continue # 如果既没有节次时间，又不是星期，直接干掉！（完美解决尾部大片空白）
                                
                            # 干掉诸如“第四周”、“备注”之类的杂物
                            if bool(re.search(r'(第.*周|日期|交表|备注|签章|签字|核对)', p_val + t_val)):
                                continue
            ...
            if class_grids:
                if "ZIP" in export_mode_type:
                    ...
                    st.success(f"🎉 成功生成！已为您把全校 {len(class_grids)} 个班级的独立表格打包成了 ZIP！")
                    st.download_button(
                    ...
                    st.info("💡 解压这个 ZIP 文件后，里面全是每个班级独立的 Excel 文件，直接拖进班主任群分发，再也没有烦人的空白行了！")
                else:
