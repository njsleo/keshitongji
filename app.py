...
<style>
    ...
    /* 美化侧边栏抽屉标题 */
    [data-testid="stSidebar"] [data-testid="stExpander"] summary p { font-weight: 600; font-size: 15px; color: #1e3c72; }
</style>
...
# ================= 侧边栏与核心控制台 =================
st.sidebar.markdown('<div style="text-align:center; padding-bottom:10px;"><h2 style="color:#1e3c72; font-weight:bold;">⚙️ 系统控制中心</h2></div>', unsafe_allow_html=True)

with st.sidebar.expander("📁 数据控制台", expanded=(st.session_state['all_sheets'] is None)):
    uploaded_file = st.file_uploader("请拖拽或点击上传 Excel (.xlsm/xlsx)", type=["xlsm", "xlsx"])
...
if st.session_state['all_sheets'] is not None:
    ...
    with st.sidebar.expander("🧑‍🏫 单人课表提取器", expanded=False):
        ...
        if st.button("🔎 提取该教师课表", use_container_width=True):
            if search_input.strip() == "": st.warning("请先输入教师姓名！")
            else:
                ...
                st.session_state['schedule_range'] = schedule_range_single

    with st.sidebar.expander("📦 批量导出工具箱", expanded=False):
        export_format = st.radio("选择打包内容与格式", [
        ...
        schedule_range_batch = "全部周次 (长表)"
        if "教师课表" in export_format:
            schedule_range_batch = st.radio("📅 教师周次选择", ["全部周次 (长表)", "仅第一周 (经典单周)"], key="sr_batch")
        ...
        if st.button("🚀 一键提取并打包下发", use_container_width=True):
            if "班主任核对表" in export_format and len(st.session_state.get('g_dates', [])) < 1:
                ...
                st.session_state['schedule_range'] = schedule_range_batch

    with st.sidebar.expander("🌐 批量课时发薪统计", expanded=False):
        scope = st.radio("📌 薪资统计范围选择", ["所有班级 (全校)", "按年级多选", "自定义勾选班级"])
        if scope == "所有班级 (全校)": target_classes = valid_classes
        elif scope == "按年级多选":
            grades = st.multiselect("挑选年级", ["高一", "高二", "高三"], default=["高三"])
            target_classes = [c for c in valid_classes if any(g in c for g in grades)]
        else:
            target_classes = st.multiselect("勾选具体的班级", valid_classes, default=valid_classes[:2])

        if st.button("💰 生成课时薪资汇总", use_container_width=True, type="primary"):
            if not target_classes: st.error("当前没有选定任何班级！")
            else:
...
if st.session_state['all_sheets'] is not None:
    ...
    if st.session_state.get('batch_export_mode'):
        ...
        if "班主任核对表" in export_mode_type:
            ...
            f_end = f_dates[1] if len(f_dates) == 2 else f_start
            ...
            with st.spinner('正在为您生成极致排版的智能核对表，这可能需要几秒钟...'):
                for s_name in valid_classes_to_search:
                    ...
                    for date, rows in blocks:
                        if date and str(date) >= str(f_start) and str(date) <= str(f_end):
                    ...
                    if target_block:
                        ...
                        for i in range(len(grid_df)):
                            ...
                            p_val = row_vals[0]; t_val = row_vals[1]
            ...
            if class_grids:
                if "ZIP" in export_mode_type:
                    ...
                    st.download_button(label=f"⬇️ 下载全校合并版 Excel", data=excel_data, file_name=f"班主任课时核对表_全校集合版_{f_start}至{f_end}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
            else:
